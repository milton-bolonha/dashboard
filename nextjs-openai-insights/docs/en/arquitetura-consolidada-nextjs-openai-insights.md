...
- **SWR**: `/api/workspace` with intelligent polling
- **Intelligent Polling**: Exponential backoff (2s → 10s) until the first tile appears
- **Fallback**: Rehydrates from `localStorage` when the session expires
...
PlanLimits := {
  ...
  pro: {
    ...
    features: ["all_templates", "unlimited_chat", "custom_templates",. 
      "bulk_upload"]
  },
...
CheckoutFlow := {
  ...
  initiate_checkout(userId, planId) → {
    session = stripe.checkout.sessions.create({
      ...
      cancel__url: `${APP_URL}/admin?upgrade=cancelled`
    })
    return session.url
  }
}
...
> 📊 **For full implementation status details**, see [`docs/STATUS-IMPLEMENTACAO.md`](./STATUS-IMPLEMENTACAO.md).
...
All documents are organized in `docs/`. See [`docs/README.md`](./README.md) for the full index.

**Main Documentation**:

- [`docs/README.md`](./README.md) — Main documentation index
- [`docs/novo-fluxo.md`](./novo-fluxo.md) — Updated operational flow (Nov/2025)
- [`docs/fluxo-tiles.md`](./fluxo-tiles.md) — Tile creation and prompt engineering flow
- [`docs/README-admin.md`](./README-admin.md) — Detailed anatomy of themes (Ade, Classic, Dash)
- [`docs/generic.md`](./generic.md) — Complete architecture blueprint
- [`docs/prompt-fluxo.md`](./prompt-fluxo.md) — Architecture visualization system
- [`docs/CHECKLIST-ADMIN.md`](./CHECKLIST-ADMIN.md) — Admin Checklist
- [`docs/plano-finalizar-14-11.md`](./plano-finalizar-14-11.md) — Finalization Plan
