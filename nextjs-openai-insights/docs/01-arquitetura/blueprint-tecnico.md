...
**Filosofia**: Sistema enxuto, inteligente e altamente escalável sem camadas desnecessárias. Foco em técnicas práticas que maximizam capacidade com mínima complexidade.

---

## 1. Core Domain Model

### 1.1 Workspace (Contexto de Trabalho)
...
- Estado unificado com rastreamento de origem (`server` | `localStorage` | `cache`)
- Timestamps para detecção de geração ativa (`generatedAt`)
- Sistema de aparência dinâmica com tokens computados

### 1.2 Entity (Unidade de Dados)
...
- Histórico limitado (`MAX_HISTORY_LENGTH=12`) para reduzir tokens
- Normalização de mensagens (roles garantidos: `assistant` | `user` | `system`)
- Clamp de conteúdo para previews (`clampTiles`, `maxChars=320`)

---

## 2. Request Engine (Motor de Requisições)
...
- Validação com Zod para type-safety em runtime
- Validação de modelos contra `VALID_MODELS` array
- Fallback automático para `DEFAULT_MODEL` com warning
...
- Detecção automática de API baseada no modelo (`isGPT5Model()`)
- Conversão de mensagens para formato correto (`convertMessagesToInput()`)
- Parâmetros dinâmicos (`max_tokens` vs `max_completion_tokens` vs `max_output_tokens`)
...
Batch := {
  size: Integer,  // BROWSER_TILE_BATCH_SIZE (configurável)
...
// Implementado: UseMock quando MOCK_OPENAI_RESPONSES=true
```

**Otimizações**:

- Batch configurável via env var
- Mock mode para desenvolvimento/testes
- Retry com backoff exponencial (2 tentativas, 400ms delay)

---

## 3. Response Normalization (Normalização Profissional)
...
normalize(raw: RawResponse) → NormalizedResponse := {
  extract_content(raw, api_type),  // Adaptador específico por API
...
// Implementado: OpenAI Multi-API Adapter
OpenAIAdapter := {
...
- Logs detalhados para debug de estrutura de resposta
- Validação de estrutura antes de extração
...
- Cookie HTTP-only apenas para `sessionId` (não dados sensíveis)
...
// Implementado: 2-Tier Strategy (atual)
// Planejado: 3-Tier Strategy (com MongoDB)
StorageStrategy := {
  tier1: {
    type: Memory,
    ttl: 30min,
...
- **LRU no localStorage**: Mantém apenas 5 workspaces recentes
- **Índice ordenado**: `insights_workspace_index` para acesso rápido
- **Última sessão**: `insights_workspace_last` para abertura rápida
- **Pruning automático**: Remove entradas antigas ao exceder limite
- **Silent failures**: Quota errors ignorados (graceful degradation)
...
// Implementado: Unified State with Source Tracking
WorkspaceState := {
...
// Implementado: Intelligent State Unification
...
- **Source tracking**: Rastreia origem dos dados para debug e decisões
- **Preservação de seleção manual**: Previne auto-switch quando usuário seleciona workspace
- **Ref para seleção**: `userSelectedSessionRef` marca seleções manuais
- **Preservação de estado local**: Mantém chat history mesmo quando servidor atualiza
...
- Para automaticamente quando tiles são encontrados
- Verifica múltiplas condições (workspace `generatedAt` + `localStorage` timestamp)
- Janela de polling aumentada para workspaces recém-gerados (2 minutos)
...
- Histórico limitado reduz tokens e custos
- Clamp de conteúdo para previews em UI
- Normalização antes de enviar para API
...
- Processamento com `processPromptVariables()`
...
- **Pre-hydration application**: Script inline aplica cor antes do React hidratar
- **suppressHydrationWarning**: Previne erros de hidratação em elementos dinâmicos
- **isMounted state**: Garante renderização consistente servidor/cliente
- **Validação de hex**: Regex `/^#[0-9A-Fa-f]{6}$/` antes de aplicar
...
- Array de modelos válidos (`VALID_MODELS`)
- Validação em runtime com fallback
- Logs de warning para modelos inválidos
...
## 11. Scalability Techniques (Enxuto mas Poderoso)

### 11.1 Memory Efficiency

```
Techniques := {
  // Deep cloning apenas quando necessário
  clone_on_write: true,
  clone_on_read: false,  // Só quando mutação possível

  // LRU eviction no localStorage
  lru_cache: {
    max_entries: 5,
  ...
  // TTL automático no servidor
  auto_purge: {
    on_read: true,  // Purge expired antes de ler
    on_write: false,
    background: false  // Não precisa, purge on-demand é suficiente
  }
}
```
...
Techniques := {
  ...
  memoize_appearance_tokens: true,  // useMemo com dependências corretas

  // Hydration optimization
  suppress_hydration_warning: true,  // Em elementos dinâmicos
  is_mounted_state: true,  // Para renderização condicional consistente
...
```
Techniques := {
  // Unificação inteligente (evita duplicação)
  unified_state: {
    single_source_of_truth: false,  // Multi-source com prioridades
    source_tracking: true,
    priority_system: true
  },

  // Preservação de seleção manual
  ...
  optimistic_updates: {
    chat_history: true,  // Update local antes de server confirm
...
- [x] Logs detalhados (API calls, response structures, state changes)
- [x] Source tracking (Workspace origin: server/localStorage/cache)
- [x] Error tracking (Warnings para modelos inválidos, fallbacks)
- [x] Latency monitoring (Logs de timing em operações críticas)
...
- [x] Validation schemas (Zod para type-safety)
...
- [ ] Background sync (Sync local → server quando offline)
...
APIRouteUpdates := {
  ...
  handler: async (request: Request) => {
    ...
    if requires_auth and not userId then
      return UnauthorizedResponse

    // Use userId for workspace isolation
    workspace = await read_workspace(sessionId, userId),
...
### 14.1 Enxutamento (Lean Architecture)
...
### 14.2 Inteligência (Smart Decisions)
...
### 14.3 Escalabilidade (Scalable Foundation)
...
### 14.4 Resilência (Resilient System)
...
- [ ] Define data models (users, workspaces, tiles, contacts, notes, usageCounters)
- [ ] Setup MongoDB connection and indexes
- [ ] Implement dual-write pattern (Memory + MongoDB)
- [ ] Create migration routine (localStorage → MongoDB on auth)
- [ ] Update APIs to use MongoDB with fallback
...
- [ ] Update MembershipProvider to read from backend
