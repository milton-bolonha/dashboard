...
const DETAILED_KEYWORDS = ['write', 'script', 'email', 'analyze', 'strategy', 'generate'];
...
export function classifyTileProfile(tile, position, totalTiles) {
  ...
  const hasDetailedKeyword = DETAILED_KEYWORDS.some(kw => titleLower.includes(kw));
  ...
  const isComplexPrompt = promptLower.includes('provide') && 
...
export function optimizeSystemPrompt(context, profile) {
  const { company, companyWebsite, solution, researchTarget, researchWebsite } = context;
