...
## 🔎 Modo de Exposição (API) — exposureMode

- **Objetivo:** controlar como a API pública expõe itens de uma Section.
- **Escopo:** somente `strategy = collection`. Em `singleton` é irrelevante; em `grouping` não se aplica.
- **Campos (SectionSchema):**
  - `exposureMode`: `"all" | "single"` (padrão: `all`)
  - `exposureSelection`: `"random" | "latest"` (usado apenas quando `exposureMode = "single"`)
- **Comportamento da API:**
  - `all`: retorna a lista completa (com paginação/filtros existentes).
  - `single/random`: retorna exatamente 1 item aleatório (MongoDB `$sample: { size: 1 }`).
  - `single/latest`: retorna exatamente 1 item mais recente (`sort { createdAt: -1 }` + `limit 1`).

Veja especificação detalhada em `section-exposure-mode.md`.
